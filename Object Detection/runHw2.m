...
%labeled_two_obj = imread('labeled_many_objects_1.png');
...
for i = 1:length(img_list)
    ...
    orig_img = imread([img_list{i} '.png']);
    output_img = recognizeObjects(orig_img, labeled_img, obj_db, '1c1');
...
for i = 1:length(img_list)
    labeled_img = imread(['labeled_' img_list{i} '.png']);
    orig_img = imread([img_list{i} '.png']);
    output_img = recognizeObjects(orig_img, labeled_img, obj_db, img_list{i});
...
for i = 1:length(img_list)
    ...
    %remove noise, please comment if not allowed
    img = imfilter(img,fspecial('gaussian', [8 8], 2), 'replicate');
    edge_img = edge(img,'canny', .15);
...
for i = 1:length(img_list)    
    ...
    rho_num_bins = sqrt (size(img , 1) * size(img , 1) + size(img , 2) * size(img , 2));
...
img_list = {'hough_1', 'hough_2', 'hough_3'};
hough_threshold = [0.33 0.04 0.41];
